...
def assess_spec_dir(spec_path):
    """
    Assess a single spec directory to determine its state in the SDD (HTML edition) workflow.
    """
    ...
    files = list(spec_dir.glob("*.html"))
    file_names = [f.name for f in files]

    spec_file = f"{seq_num}-spec-{feature_name}.html"
    tasks_file = f"{seq_num}-tasks-{feature_name}.html"
    audit_file = f"{seq_num}-audit-{feature_name}.html"
    validation_file = f"{seq_num}-validation-{feature_name}.html"

    # Check for any questions file matching the pattern [NN]-questions-[N]-[feature].html
    has_questions = any(re.match(rf'^{seq_num}-questions-\d+-{re.escape(feature_name)}\.html$', name) for name in file_names)
    ...
    if not state["files_found"]["spec"]:
        ...
        # Check if tasks are parents only (TBD marker: literal text or data-tbd attribute)
        tasks_path = spec_dir / tasks_file
        has_tbd = False
        try:
            with open(tasks_path, 'r', encoding='utf-8') as f:
                if re.search(r'data-tbd="true"|\bTBD\b', f.read()):
                    has_tbd = True
        except Exception:
            pass
        ...
        # Check audit gates for FAIL (attribute takes precedence, literal text as fallback)
        audit_path = spec_dir / audit_file
        audit_failed = False
        try:
            with open(audit_path, 'r', encoding='utf-8') as f:
                if re.search(r'data-overall-status="FAIL"|\bFAIL\b', f.read()):
                    audit_failed = True
        except Exception:
            pass
        ...
        # Check task completion via data-status attributes on individual task items
        tasks_path = spec_dir / tasks_file
        incomplete_tasks = False
        try:
            with open(tasks_path, 'r', encoding='utf-8') as f:
                content = f.read()
                # Look for incomplete task status markers
                if re.search(r'data-status="pending"', content) or re.search(r'data-status="in-progress"', content):
