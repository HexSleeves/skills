...
These patterns can create false drama when repeated or used without a real distinction. Keep a
contrast when both sides are accurate and the distinction matters.
...
An inanimate subject can hide the responsible actor, but it is not automatically wrong. Name the
person or team when responsibility matters; keep conventional technical subjects such as
"the compiler reports" or "the test fails" when they are precise.
...
Prefer active voice when readers need to know who acted. Passive voice remains useful when the
actor is unknown, irrelevant, deliberately omitted, or less important than the result.
...
## Sentence starters to review

| Pattern | Fix |
|---------|-----|
| Repeated question-word openers | Lead with the subject or verb when that is clearer |
| Habitual paragraph openings with "So" | Remove it when it adds no transition |
| Repeated "Look," openings | Keep only when the direct address fits the author's voice |

Repeated question-word openers can become a crutch. Restructure them when a direct subject makes
the sentence clearer; keep them when the grammar or intended emphasis calls for them.

## Rhythm Patterns

| Pattern | Fix |
|---------|-----|
| Repeated three-item lists | Keep real groups of three; vary formulaic grouping |
| Questions answered immediately | Keep when useful; cut a staged rhetorical setup |
| Every paragraph ends punchily | Vary endings when the rhythm becomes mechanical |
| Em dashes | Follow the author's style and the document's punctuation rules |
...
| Pattern | Problem |
|---------|---------|
| Unsupported extremes (every, always, never, everyone, everybody, nobody) | Use a narrower claim unless the evidence supports the absolute. |
| Filler modifiers (for example, "really," "just," or "actually") | Remove empty emphasis; keep modifiers that carry meaning. See phrases.md. |
